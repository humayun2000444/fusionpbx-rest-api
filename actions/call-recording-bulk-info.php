...
function do_action($body) {
    ...
    // Option 2: Download by date range
        ...
        // Validate dates (accepts YYYY-MM-DD or YYYY-MM-DD HH:mm)
        if (!preg_match('/^\d{4}-\d{2}-\d{2}/', $start_date) || !preg_match('/^\d{4}-\d{2}-\d{2}/', $end_date)) {
            return array("error" => "Invalid date format. Use YYYY-MM-DD or YYYY-MM-DDTHH:mm");
        }

        // If only date provided, add start/end of day
